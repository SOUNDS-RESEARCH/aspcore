...
filt = create_filter(
    num_in=num_in, num_out=num_out, ir_len=ir_len, sum_over_input=True, dynamic=True
)
    ...
    for out_ch in range(num_out):
        for in_ch in range(num_in):
            for i in range(ir_len):
                filtered_signal[out_ch, n] += (
                    ir[n, in_ch, out_ch, i] * buffered_input[in_ch, ir_len + n - i - 1]
                )

print(
    f"Both implementations are equal: {np.allclose(filtered_signal, filtered_signal_dyn)}"
)
