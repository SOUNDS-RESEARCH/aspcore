r"""Functions associated with perfect periodic sequences.
...
def decorrelate(sig, pseq, v=0):
    """Identify an LTI system from a PSEQ input signal.
...
def create_pseq(seq_len: int):
    """Create a perfect sweep with constant magnitude spectrum.
...
def create_pseq_lowfreq(seq_len: int, sr: int, max_pseq_freq: int):
    """Create pseq with a maximum frequency to be used at a higher samplerate.
    ...
    # FFT-based upsampling: for a periodic input of length seq_len_short, treating
    # it as the DFT of one period and zero-padding to length seq_len gives a
    # bandlimited interpolation that exactly preserves the original samples at
    # multiples of sampling_factor (i.e. seq_upsampled[k*M] == short[k]).
    # samplerate.resample with sinc kernels is approximate even for periodic
    # inputs (transition-band rolloff causes ~15-20% sample-by-sample deviation).
    short = create_pseq(seq_len_short)[0, :]
    seq_upsampled = spsig.resample(short, seq_len)
    return seq_upsampled[None, :]


def create_pseq_random_phase(seq_len: int, rng=None):
    """Create a perfect sweep with random phase and constant magnitude spectrum.
...
def create_shifted_pseq(pseq, num_channels, rir_len):
    """Create a shifted PSEQ meant for MISO system identification.
    ...
    shifted_pseq = np.concatenate(
        [np.roll(pseq, i * rir_len) for i in range(num_channels)], axis=0
    )
    return shifted_pseq


def verify_pseq(pseq, plot=False, samples_to_show=300):
    """Verify that a sequence is a perfect sequence.
    ...
    info = {
        ...
        "Perfect periodic autocorrelation": np.abs(autocorr[0]) > 1e-3
        and np.allclose(autocorr[1:], 0),
    }
    ...
    if plot:
        ...
        plt.figure()
        joined_signal = np.concatenate(
            (pseq[0, -samples_to_show:], pseq[0, :samples_to_show])
        )
